...
// =====
const LISTEN_OPTS = {
   capture: true, // Just to trigger on the Capture phase; Note: (1) Capture phase -> (2) Target phase -> (3) Bubbling phase
   passive: true 
};

// var previousInputEle; // case: after user completes typing an input, just goes to the next
// =====

var ENABLE_NETWORK_LOGGING = true; // Controls network logging.
var ENABLE_CONSOLE_LOGGING = false; // Controls console logging.
var LOG_VERSION = 'wasteSpace_toRemove';           // Labels every entry with version: "0.1".

// These event types are intercepted for logging before jQuery handlers.
var EVENT_TYPES_TO_LOG = {
  mousedown: true,
  keydown: true,
  // focus: true, // with capture=true, detects if window is clicked as well
  // blur: true,
  focusin: true,
  focusout: true
};
...
function hookEventsToLog() {
  ...
  for (var event_type in EVENT_TYPES_TO_LOG) {
    ...
    document.addEventListener(event_type, logEvent, LISTEN_OPTS);
    // document.addEventListener(event_type, logEvent); // <<<<<<<<<<<<<< NOTE: Intercept events after bubbling. Not sure what's the impact
  }
}
...
function logEvent(event, customName, customInfo) {
  ...
  if (eventName === 'keydown') {
    if (event.which === 13){ // 13 = 'Enter' key
      ...
      console.log('"Enter" detected, value: ', document.getElementsByClassName('mat-active')[0].innerText);
    }
  } else if (eventName === 'focusout') {
    var focusOutEle = event.target;
    if (focusOutEle.nodeName === 'INPUT') { //'input'.toUpperCase()
      console.log('FOCUSOUT input"s value: ', focusOutEle.value);
    }
  }
  ...
  if (eventName === 'focusin') {
    ...
    switch(actEle.nodeName){
      case 'MAT-OPTION': //'mat-option'.toUpperCase():
        console.log('mat-option detected');
        break;
      case 'BUTTON': //'button'.toUpperCase():
        if (actEle.id === 'search') {
          console.log('search button')
        } else {
          console.log('generic button detected (guessing datepicker)', "class", actEle.classList[0]);
          console.log('date: ', actEle.parentElement.parentElement.parentElement.getElementsByClassName("mat-input-element")[0].value)
        }
        break;
      case 'INPUT': //'input'.toUpperCase():
        console.log('input detected, value: ', actEle.value);
        break;
      default:
        console.error('Unexpected: actEle.nodeName=',actEle.nodeName);
    }
  }
...
// docs.google.com/forms/d/e/1FAIpQLSdUlXCvQWKiTdY7D7T-ubFsxsKlcT_J2xaWsdtxQBW7He6qvg/viewform
function sendNetworkLog(
    uid,
    time,
    eventname,
    ...
    state,
    log_version) {
  // var formid = "`e/1FAIpQLSdUlXCvQWKiTdY7D7T-ubFsxsKlcT_J2xaWsdtxQBW7He6qvg";
  // var data = {
  //   "entry.1214439124": uid,
  //   "entry.405434972": time,
  //   "entry.470159027": eventname,
  //   "entry.1909736677": target,
  //   "entry.66873019": info,
  //   "entry.1929668987": state,
  //   "entry.1787489642": log_version
  // };
  // var params = [];
  // for (key in data) {
  //   params.push(key + "=" + encodeURIComponent(data[key]));
